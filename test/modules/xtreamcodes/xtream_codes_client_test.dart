  ...
  group('XtreamCodesClient', () {
    ...
    group('Stream URL generation', () {
      ...
      test('getLiveStreamUrl should generate correct URL with default format', () {
        ...
        expect(url, equals('http://server.example.com:8080/live/testuser/testpass/12345.m3u8'));
      });
      ...
      test('getMovieStreamUrl should generate correct URL with default extension', () {
        final url = client.getMovieStreamUrl(credentials, '67890');
        
        expect(url, equals('http://server.example.com:8080/movie/testuser/testpass/67890.mp4'));
      });

      test('getMovieStreamUrl should generate correct URL with custom extension', () {
        final url = client.getMovieStreamUrl(credentials, '67890', extension: 'mkv');
        
        expect(url, endsWith('/67890.mkv'));
      });

      test('getSeriesStreamUrl should generate correct URL with default extension', () {
        final url = client.getSeriesStreamUrl(credentials, '11111');
        
        expect(url, equals('http://server.example.com:8080/series/testuser/testpass/11111.mp4'));
      });

      test('getSeriesStreamUrl should generate correct URL with custom extension', () {
        final url = client.getSeriesStreamUrl(credentials, '11111', extension: 'avi');
